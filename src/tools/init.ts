  ...
  async execute(params) {
    ...
    return withGuard(async () =>
      withLock(initMutex, async () => {
        ...
        if (await ResearchFS.exists(statePath)) {
          ...
          if (state) {
            ...
            const agentsMdExists = await fs
              ...
              .catch(() => false);
            if (!agentsMdExists) {
              // AGENTS.md lives at the scope root (not under .research/), so it
              // must never go through ResearchFS.writeMd (which registers files
              // in the .research index and would reject an out-of-scope path).
              await Bun.write(agentsMdPath, generateAgentsMd(state));
            } else {
              // K8: AGENTS.md generated by the upstream holos-research plugin
              // references research_wiki (removed in this derivative). Regenerate
              // the whole file once so the static rules match the Scholens-based
              // literature workflow.
              const existingAgentsMd = await Bun.file(agentsMdPath)
                .text()
                .catch(() => "");
              if (existingAgentsMd.includes("research_wiki")) {
                await Bun.write(agentsMdPath, generateAgentsMd(state));
              }
            }
        ...
        const projectRoot = scopeDir();
        // Scholens-based layout: literature surveys + research-gap notes are
        // plain markdown at the scope root (docs/surveys/, docs/gaps.md), never
        // a local literature store under .research/.
        await fs.mkdir(path.join(projectRoot, "docs", "surveys"), { recursive: true });
        ...
        try {
          ...
          // Direct Bun.write: AGENTS.md is a scope-root file, not part of the
          // .research index (ResearchFS.writeMd would reject the out-of-scope path).
          const agentsMdPath = path.join(projectRoot, "AGENTS.md");
          const agentsMdContent = generateAgentsMd(state);
          await Bun.write(agentsMdPath, agentsMdContent);
        ...
        const lines = [
          ...
          "  docs/surveys/ (literature surveys — see lit-knowledge skill)",
          "  docs/gaps.md (research gaps — agent-maintained)",
          "  docs/scholens-project.md (scope ↔ Scholens project binding)",
          "",
