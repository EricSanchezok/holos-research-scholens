...
> **holos-research-scholens 派生差异（本文档为历史设计稿，描述上游 v2.1 愿景）**：
> 本仓库是 `yzxoi/holos-research` 的派生插件。**文献子系统已从本地 `.research/literature/` 迁移到 Scholens**：
> - `research_wiki` 工具、`src/resolve/`、`src/match/`、`literature/` 目录、gap_map/edges/LIT_CONTEXT/references.bib 已删除。
> - 论文收录/检索/标注/引用改由 `scholens` MCP（`mcp__scholens__*`）承担；综述写入 `docs/surveys/`、缺口写入 `docs/gaps.md`、scope↔Scholens project 绑定写入 `docs/scholens-project.md`。
> - 下文凡提到 wiki / literature 目录 / research_wiki 的段落，均为上游历史设计的残留描述，不再对应本仓库实现。
...
| ~~`research_wiki`~~ | **已移除（派生插件）** — 文献工作流改由 scholens MCP 承担；`used_wiki_refs` 保留为 phase_run 兼容字段 |
...
| Citation existence | Scholens ingest / paper audit | citation blocked or audit fail |
